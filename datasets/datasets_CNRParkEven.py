'''此程序建立了CNRPark子数据集even中的所有patch以及对应label的数据集 （CNRPark的子数据集even）'''
...
class Patchs(Dataset):
    ...
    def __init__(self, root, tf, mode):   # mode:模式（训练 or 测试）   tf:transform对图像采取变换
        ...
        self.images, self.labels = self.load_txt('data/LABELS/even.txt')
...
train_datasets = Patchs('data/CNRPark', data_transforms['train'], mode='train')
test_datasets = Patchs('data/CNRPark', data_transforms['test'], mode='test')
